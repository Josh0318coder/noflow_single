...
class VideosDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.ref_augmentation = ref_augmentation
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            # Apply augmentation to reference images (before color transform)
            if self.ref_augmentation is not None:
                I_reference_video = self.ref_augmentation(I_reference_video)
                I_reference_video_real = self.ref_augmentation(I_reference_video_real)
...
class VideosDataset_ImageNet(data.Dataset):
    def __init__(
        ...
        self.distortion_transform = Distortion_with_flow_cpu()  # 只使用 CPU 版本
    ...
    def __getitem__(self, index):
        ...
        # Apply augmentation to reference images (before other transforms)
        if self.ref_augmentation is not None:
            I_reference_video = self.ref_augmentation(I_reference_video)
            I_reference_video_real = self.ref_augmentation(I_reference_video_real)
        ...
        # ✅ 修改6: 返回值從 10 個元素改為 5 個元素
        # 移除: I1 (前一幀), flow_forward, mask, placeholder, "holder"
        # 保留: I2 (當前幀), I_reference_output, self_ref_flag, 路徑信息
