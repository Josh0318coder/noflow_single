...
class SwinTExCoWithVisualization:
    ...
    def __preprocess_frame(self, img_path):
        """CPU: load image and preprocess into tensors (for prefetch)"""
        image = Image.open(img_path).convert('RGB')
        gray_image = rgb_to_grayscale(image)
        large_IA_l = ToTensor()(RGB2Lab()(gray_image)).unsqueeze(0)[:, 0:1, :, :]
        IA_l = self.processor(gray_image).unsqueeze(0)[:, 0:1, :, :]
        return large_IA_l, IA_l
    ...
    def __process_sample(self, large_IA_l, IA_l, I_reference_lab, features_B, return_intermediates=False):
        """
        Process single frame (GPU inference)

        Args:
            large_IA_l: L channel tensor at original size [1, 1, H, W] (CPU)
            IA_l: L channel tensor at 224x224 [1, 1, 224, 224] (CPU)
        ...
        """
        large_IA_l = large_IA_l.to(self.device)
        IA_l = IA_l.to(self.device)

        with torch.no_grad():
            I_current_ab_predict, nonlocal_BA_lab, similarity_map = frame_colorization(
                IA_l,
                I_reference_lab,
                features_B,
                self.embed_net,
        ...
        # nonlocal_BA_lab: NonlocalNet 直接輸出，尺寸 = image_size / 4
        # 對於 224×224 輸入 → 56×56
        nonlocal_ab = nonlocal_BA_lab[:, 1:3, :, :]  # [1, 2, H/4, W/4]
        raw_h, raw_w = nonlocal_ab.shape[2], nonlocal_ab.shape[3]

        # Target L 下採樣至 56×56
        IA_l_small = torch.nn.functional.interpolate(
            IA_l,
            size=(raw_h, raw_w),
            mode="bilinear",
            align_corners=False
        ...
        # --- Fusion 視覺化（target L + warped ab → RGB 56×56）---
        fusion_raw_rgb = tensor_lab2rgb(
            torch.cat((uncenter_l(IA_l_small), uncenter_ab(nonlocal_ab)), dim=1)
        ).cpu()
        fusion_raw_rgb_np = (fusion_raw_rgb.squeeze(0).numpy().transpose(1, 2, 0) * 255).astype(np.uint8)

        # --- Warp 視覺化（same as fusion but explicit label）---
        # nonlocal_BA_lab 的 ab 即為 warp 後的 reference 顏色
        # 與 fusion_raw_rgb_np 相同，但作為獨立輸出提供
        warp_rgb_np = fusion_raw_rgb_np.copy()

        # --- 原始相似度圖（56×56）---
        raw_similarity_np = similarity_map.squeeze().cpu().numpy()  # [H/4, W/4]
        ...
        return (IA_predict_rgb, fusion_raw_rgb_np, large_similarity, raw_similarity_np, warp_rgb_np)
    ...
    def predict_image(self, image, ref_image):
        ...
        large_IA_l = ToTensor()(RGB2Lab()(image)).unsqueeze(0)[:, 0:1, :, :]
        IA_l = self.processor(image).unsqueeze(0)[:, 0:1, :, :]
        results = self.__process_sample(large_IA_l, IA_l, I_reference_lab, features_B, return_intermediates=False)
        return results[0]
    ...
    def predict_dataset(self, input_root, output_root,
        ...
        Args:
            ...
            save_fusion: Save nonlocal fusion ab visualization (target L + warped ab, 56×56)
            ...
            save_warp: Save NonlocalNet warp output as RGB (56×56)
        ...
        for scene_name in tqdm(scene_folders, desc="Processing scenes"):
            ...
            if save_warp:
                warp_output_path = os.path.join(output_root, f"{scene_name}_warp")
                os.makedirs(warp_output_path, exist_ok=True)

            # Process each frame
            need_intermediates = save_fusion or save_similarity or save_raw_similarity or save_warp

            with ThreadPoolExecutor(max_workers=1) as executor:
                # Pre-submit all frames: while GPU runs frame N, CPU prefetches frame N+1
                futures = [executor.submit(self.__preprocess_frame, p) for p in image_files]

                for img_path, future in tqdm(zip(image_files, futures), total=len(image_files), desc=f"  {scene_name}", leave=False):
                    large_IA_l, IA_l = future.result()
                    ...
                    if need_intermediates:
                        colorized, fusion_rgb, similarity, raw_similarity, warp_rgb = results
                    else:
                        colorized = results[0]
                    ...
                    # Save warp visualization (NonlocalNet output, target L + warped ref ab, 56×56)
                    if save_warp:
                        warp_path = os.path.join(warp_output_path, filename)
                        Image.fromarray(warp_rgb).save(warp_path)
        ...
        if save_warp:
            print(f"   Warp visualization (56×56): {output_root}/*_warp/")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_warp', action='store_true',
                       help='Save NonlocalNet warp output as RGB (target L + warped ref ab, 56×56)')
    ...
    if args.mode == 'single':
        ...
        if args.save_warp:
            print(f"  Saving NonlocalNet warp output - 56×56 RGB")
